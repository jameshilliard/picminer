...
BOOL StringToIPAddress(BYTE* str, IP_ADDR* IPAddress) {
	BYTE i;
	for(i=0;i<4;i++) {
		IPAddress->v[i] = atoi(str); 
		while(*str)	if(*str++=='.') break;
	}
	return FALSE;
}

BOOL ROMStringToIPAddress(ROM BYTE* rst, IP_ADDR* IPAddress) {
	BYTE i, *str, sta[32];
	str = sta;
	strcpypgm2ram(str,rst);
	for(i=0;i<4;i++) {
		IPAddress->v[i] = atoi(str); 
		while(*str)	if(*str++=='.') break;
	}
