...
/*
extern void bin2hex(BYTE *a, BYTE c);

static void Outso(void * bf, BYTE num) {
	BYTE sta[3], *pt = bf;
	while(num--)	{ bin2hex(sta, *pt++); putsUART(sta); }
}
*/
...
//	putrsUART("\r\n IPmm: "); Outso((BYTE *)&Cache.IPAddr.Val,4); putcUART(' ');
				return TRUE;
			}
#endif
...
/*****************************************************************************
  ...
  ***************************************************************************/
#ifdef STACK_CLIENT_MODE
// void ARPClrCache(void) {; }	//{ Cache.IPAddr.Val = 0ul; }

void ARPResolve(IP_ADDR* IPAddr) {
	BYTE c;
	ARP_PACKET packet;
	static IP_ADDR ipo=0;
    

//	putrsUART("\r\n "); Outso((BYTE *)&Cache.IPAddr.Val,4); putcUART(' '); Outso((BYTE *)&Cache.MACAddr.v[0],6); putcUART(' ');
//	putrsUART("\r\n "); Outso((BYTE *)&IPAddr->Val,4); putcUART(' ');

	if(( ipo.Val == IPAddr->Val) && (Cache.IPAddr.Val) ) 		return;

	ipo.Val = IPAddr->Val;
	Cache.IPAddr.Val = 0;
/*
	if((Cache.IPAddr.Val) && AppConfig.MyGateway.Val)	return;
	if((Cache.IPAddr.Val) == IPAddr->Val)				return;
*/
...
//	packet.TargetIPAddr = *IPAddr;
//	putrsUART("\r\n "); Outso((BYTE *)&IPAddr->Val,4); putcUART(' '); Outso((BYTE *)&packet.TargetIPAddr.Val,4); putcUART(' '); Outso((BYTE *)&Cache.IPAddr.Val,4); putcUART(' ');
    ARPPut(&packet);
}
#endif
...
BOOL ARPIsResolved(IP_ADDR* IPAddr, MAC_ADDR* MACAddr) {
	BYTE c;
   	...
   	if((Cache.IPAddr.Val == IPAddr->Val) ||  ((Cache.IPAddr.Val == AppConfig.MyGateway.Val) && ((AppConfig.MyIPAddr.Val ^ IPAddr->Val) & AppConfig.MyMask.Val))) {
       	*MACAddr = Cache.MACAddr;       return TRUE;
   	}

//	if(Cache.IPAddr.Val == IPAddr->Val)	return TRUE;
    return FALSE;
}
#endif
