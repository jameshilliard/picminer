...
/*
extern IP_ADDR String2IP(char *string);
void DNSResolve(BYTE* Hostname, BYTE Type) {
	ResolvedInfo.IPAddr = String2IP(Hostname);
...
BOOL DNSIsResolved(IP_ADDR* HostIP) {
	...
	static BYTE 		who	=1;	////// added by me to avoid DNSs swap
//	static IP_ADDR		IPs; ////// added by me to avoid DNSs swap
	BYTE 				i;
	WORD_VAL			w;
...
//			ARPResolve(&AppConfig.PrimaryDNSServer);
			ARPResolve(who ? &AppConfig.PrimaryDNSServer:&AppConfig.SecondaryDNSServer);
			StartTime = TickGet();
...
//			if(!ARPIsResolved(&AppConfig.PrimaryDNSServer, &ResolvedInfo.MACAddr)) {
			if(!ARPIsResolved(who ? &AppConfig.PrimaryDNSServer:&AppConfig.SecondaryDNSServer, &ResolvedInfo.MACAddr))	{
...
//			ResolvedInfo.IPAddr.Val = AppConfig.PrimaryDNSServer.Val;
			ResolvedInfo.IPAddr.Val = who ? AppConfig.PrimaryDNSServer.Val:AppConfig.SecondaryDNSServer.Val;
		...
		case DNS_QUERY:
			if(!UDPIsPutReady(MySocket))			break;
...
//*!*/		DNSPutString(DNSHostName);
		...
		case DNS_GET_RESULT:
			if(!UDPIsGetReady(MySocket))	{
				if(TickGet() - StartTime > DNS_TIMEOUT)			smDNS = DNS_FAIL;
				break;
			}
			...
			while(DNSHeader.Answers.Val--)		{
				...
				else while(DNSAnswerHeader.ResponseLen.Val--) UDPGet(&i);
			}
