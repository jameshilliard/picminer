...
namespace Microchip
{
    ...
    public class MPFS2Builder : MPFSBuilder
    {
            ...
            try
            {
                ...
                switch (format)
                {
                    case MPFSOutputFormat.C18:
                        ...
                        break;
                    case MPFSOutputFormat.C32:
                        w = new MPFS2C32Writer(localPath + localFile);
                        break;
                    case MPFSOutputFormat.ASM30:
        ...
        private class MPFS2C18Writer : MPFS2Writer
        {
            ...
            public override void Write(byte data)
            {
                if (ImageLength % 1024 == 0 && ImageLength != 0)
                    fout.Write("\r\n};\r\nROM BYTE MPFS_" +
                        Convert.ToString((ImageLength / 1024), 16).PadLeft(6, '0') +
                        "[] = \r\n{");
            ...
            public override void Close()
            {
                fout.Write(
                    ...
                    " * End of MPFS\r\n" +
                    " **************************************************************/\r\n" +
                    "#endif // #if defined(STACK_USE_MPFS2) && !defined(MPFS_USE_EEPROM) && !defined(MPFS_USE_SPI_FLASH)\r\n"
                );
                fout.Flush();
                fout.Close();
            }
        ...
        private class MPFS2C32Writer : MPFS2Writer
        {
            #region Fields
            protected StreamWriter fout;
            #endregion

            public MPFS2C32Writer(String filename)
            {
                if (!filename.EndsWith(".c", StringComparison.OrdinalIgnoreCase))
                    filename += ".c";
                fout = new StreamWriter(filename, false, Encoding.ASCII);
